...
ResultDict = dict[str, dict[str, str]]
...
def cmd_readme_update() -> None:
    ...
    summer_2027_listings = filter_summer(listings, "2027", earliest_date=1748761200)

    # Generate main README with active listings only
    embed_table(summer_2027_listings, "positions/summer_2027_internships.md", active_only=True)

    # Generate separate README for inactive listings (same header/ads structure)
    embed_table(summer_2027_listings, "positions/inactive_roles.md", inactive_only=True)

    offseason_listings = filter_off_season(listings)
    embed_table(offseason_listings, "positions/off_season_internships.md", off_season=True)
...
def cmd_contribution_process(event_file: str) -> None:
    ...
    try:
        with open("tools/data/listings.json") as f:
        ...
        with open("tools/data/listings.json", "w") as f:
