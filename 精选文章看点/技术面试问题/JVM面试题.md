...
* 方法区（Method Area）：用于存储已被虚拟机加载的类信息、常量、静态变量、即时编译后的代码等数据。在 Java 8 之后，其实现由永久代（PermGen）变更为元空间（Metaspace）。
## 3.深拷贝和浅拷贝
...
## 7.JVM中的元空间（Metaspace）会发生垃圾回收吗

在 Java 8 之前，永久代（Permanent Generation）如果满了或者超过临界值，会触发 Full GC。Java 8 彻底移除了永久代，取而代之的是元空间（Metaspace）。元空间使用的是本地内存（Native Memory），其大小不再受 JVM 堆大小限制，但依然会发生垃圾回收。当元空间使用的内存达到高水位线（MetaspaceSize）时，会触发 GC 来卸载不再使用的类。
...
* G1(Garbage First)收集器： 面向服务端的垃圾收集器，旨在取代 CMS。G1 将堆内存划分为多个大小相等的独立区域（Region），实现了可预测的停顿时间模型。它基于“标记-整理”算法，不会产生内存碎片。
* ZGC (Z Garbage Collector)： JDK 11 引入的超低延迟垃圾收集器，旨在跨越 TB 级内存也能将停顿时间控制在 10ms 以内（JDK 16+ 甚至在 1ms 以内）。它基于彩色指针（Colored Pointers）和读屏障（Load Barriers）技术。
## 11.详细介绍一下 CMS 垃圾回收器?
